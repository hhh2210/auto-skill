...
| Verify benchmark cleaning follows `notes/benchmark_flow.md` | `uv run python scripts/data/audit_benchmark_flow.py ...` returns `status=ok`, no errors/warnings for checked-in MVP, 30WB/20PB expanded artifacts, MIMO subset, and max-available Qwen artifacts. The audit checks artifact-level flow/provenance, strict input-boundary fields, private-row binding, train/heldout separation, generated-output identity, and generation-prompt reconstruction from visible source task/materials when job rows are supplied. | Done for artifact-level flow/provenance audit; semantic leakage remains a sampled/heuristic risk |
...
| Avoid model monoculture for research claims | `runs/mvp_metrics.heldout2.current.summary.json` sees Qwen and MIMO in eval model inventory. Current MVP readiness has complete model identity and warns only that canonical artifacts are Qwen-only; full readiness still lacks model identity for the 16 unscored PresentBench official rows. | Partial |
