...
| Verify benchmark cleaning follows `notes/benchmark_flow.md` | `uv run python scripts/data/audit_benchmark_flow.py ...` returns `status=ok`, no errors/warnings for checked-in MVP, 30WB/20PB expanded artifacts, MIMO subset, and max-available Qwen artifacts. | Done for data-flow/leakage audit |
